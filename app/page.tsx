          ...
          <section id="contact" className="py-20 flex flex-col items-center gap-12">
            ...
            <div className="block sm:hidden relative w-full overflow-hidden my-4 py-2">
              ...
              <div className="flex gap-4 animate-marquee py-2 select-none" style={{ "--marquee-speed": "12s" } as React.CSSProperties}>
                ...
                ].reduce((acc, link) => [...acc, link, link], [] as any[]).map((w, idx) => (
                  <a
                    key={idx}
                    href={w.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 py-3 px-5 rounded-2xl shrink-0 transition-all duration-300 shadow-md shadow-black/5"
                    style={{ background: `${w.color === "#ffffff" ? "rgba(255,255,255,0.08)" : w.color + "12"}`, border: `1px solid ${w.color === "#ffffff" ? "rgba(255,255,255,0.15)" : w.color + "25"}` }}
                  >
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ background: w.color === "#ffffff" ? "linear-gradient(135deg,#1a1a1a,#000000)" : `linear-gradient(135deg, ${w.color}, ${w.color}dd)` }}>
                      {w.icon}
                    </div>
                    <div>
                      <p className="text-white font-bold text-xs leading-none">{w.name}</p>
                      <p className="text-white/40 text-[10px] mt-0.5 leading-none">{w.label}</p>
                    </div>
                  </a>
                ))}
              </div>
