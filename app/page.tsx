      ...
      <main className="relative min-h-screen overflow-x-hidden bg-background text-foreground" style={{ fontFamily: "'Inter',sans-serif" }}>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <section id="contact" className="py-20 flex flex-col items-center gap-12">
            ...
            {/* Mobile/Phone: Singly vertical stack list */}
            <div className="block sm:hidden w-full max-w-sm mx-auto flex flex-col gap-3 px-4 mt-6">
              {[
                {
                  name: "Telegram",
                  href: "https://t.me/YourTelegramUsername",
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-4 py-4 px-6 rounded-2xl w-full transition-all duration-300 shadow-md shadow-black/5"
                  style={{ background: `${w.color === "#ffffff" ? "rgba(255,255,255,0.06)" : w.color + "0d"}`, border: `1px solid ${w.color === "#ffffff" ? "rgba(255,255,255,0.12)" : w.color + "20"}` }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = w.color === "#ffffff" ? "rgba(255,255,255,0.12)" : w.color + "1a"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = w.color === "#ffffff" ? "rgba(255,255,255,0.06)" : w.color + "0d"; }}
                >
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0" style={{ background: w.color === "#ffffff" ? "linear-gradient(135deg,#1a1a1a,#000000)" : `linear-gradient(135deg, ${w.color}, ${w.color}dd)` }}>
                    {w.icon}
                  </div>
                  <div className="text-left">
                    <p className="text-white font-bold text-sm leading-none">{w.name}</p>
                    <p className="text-white/40 text-xs mt-1.5 leading-none transition-colors" style={{ color: `${w.color}cc` }}>{w.label}</p>
                  </div>
                </a>
              ))}
            </div>
